...
//	extract-key  search dump output for x'<96 hex>' pattern strings (extract.go)
...
subcommands:
  dump         wcctl dump -pid <PID> [-out DIR] [-full] [-shared] [-chunk N] [-meta]
  extract-key  wcctl extract-key [-in <dump dir | single binary file>] [-data-dir <folder>]
